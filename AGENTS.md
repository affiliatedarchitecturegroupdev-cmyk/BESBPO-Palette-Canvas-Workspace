...
- **Browser evidence must run against a freshly seeded DB, after `npm test`.**
  `npm test` truncates every domain table, so browsing first and testing after
  leaves the app pointed at an empty database, and a pass recorded from the
  stale fixtures describes rows that no longer exist. The order is: `npm test`,
  then seed, then browse. `apps/api/src/seed/seed.ts` re-applies on demand.
- **The seed is not a clean fixture after e2e has run.** The comms conversion
  test flips "Internal production" to `external` and nothing resets it, so a
  browser pass on a tested DB cannot show the §11.2 internal-channel boundary.
  Assert visibility boundaries against a purpose-made fixture, not on whatever
  the DB happens to hold.
- **Every seed identity is `@besbpo.example`** (plus `client-a@nimbus.example`).
  These are all in *one* org — so a "cross-org" claim needs the org actually
  checked, not just two different email domains. The dev DB carries rows from
  other orgs (e.g. an e2e-created `*@test.example`), which is why an endpoint
  can look correctly scoped when it is not.
- **Prove access with the HTTP status, not with a row you found in the DB.**
  During the N2.2a pass a `SELECT` that returned a meeting title looked like
  proof a client could read it, and a first read of `listMeetings` looked like a
...
- **`scripts/loc.sh` counts `docs/`.** A ledger-only commit moves the headline
  ...
  with no code change.
- **`ops/drift/latest.md` is tracked.** It is regenerated by
  `scripts/drift-check.js`, so refresh it in the same commit that changes the
  ledger row count or the checked-in copy disagrees with the ledger it
  describes.
