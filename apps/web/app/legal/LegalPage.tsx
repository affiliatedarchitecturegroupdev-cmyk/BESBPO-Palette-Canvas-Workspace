  ...
  return (
    <>
      ...
      <main style={{ maxWidth: 780, margin: '0 auto', padding: 'clamp(40px, 6vw, 72px) clamp(16px, 4vw, 48px)' }}>
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(30px, 4.5vw, 44px)', margin: '10px 0 8px' }}>{title}</h1>
        <p style={{ color: 'var(--ink-dim)', fontSize: 15, lineHeight: 1.7, margin: '16px 0 28px' }}>{intro}</p>
...
export function Clause({ heading, children }: { heading: string; children: ReactNode }) {
  return (
    <section style={{ margin: '0 0 28px' }}>
      <h2 style={{ fontFamily: 'var(--font-display)', fontSize: 20, margin: '0 0 10px' }}>{heading}</h2>
