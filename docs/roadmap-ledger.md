...
| A-14 | Collaboration parity blocks (timeline/DnD/subtasks/global search) | in-review | branch `n1-email-transport-auth-chain` | DnD moves + global search done under N2.2 (374 passed / 0 failed; browser pass). Timeline/Gantt, subtasks, and swimlanes remain (N2.2a) | Partial by design: moves and search ship against the existing §9 schema; subtasks need `parent_item_id`, which no migration provides yet. The row stays open until those land |
...
| N2.2 | A-14 board UI: DnD board moves, global search over the V2 §9 model. Timeline/Gantt + subtasks are **not** in this slice — `parent_item_id` has no schema | SPA-01 tokens | e2e: moves persist, reorder is gap-free, guest 403; browser: ⌘K search returns cross-board hits |
| N2.2a | A-14 follow-up: timeline/Gantt, subtasks (`parent_item_id`), swimlanes | N2.2 (needs a migration) | e2e: subtask parent/child round-trip; timeline ordering |
