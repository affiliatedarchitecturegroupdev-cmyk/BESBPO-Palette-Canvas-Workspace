...
| A-02 | Email verification + login/session | in-review | branch `n1-identity-chain` | e2e: unverified login 403, reuse 410, expired token 410, resend throttle, unknown-address non-disclosure; 354/354 | N1.2; delivery step still awaits the §0.2 provider decision (ADR-0002 D1) — the flow is complete and green against the outbox fallback |
| A-03 | Password reset/change + revocation | in-review | branch `n1-identity-chain` | e2e: reset round-trip, weak 400, reuse/expired 410, unknown 401, sessions revoked, change-password keeps caller, revoke-others; 354/354 | N1.3; all paths audited |
...
5. **Identity hardening** — header auth remains, and the A-block still stacks
   ...
   and green on branch `n1-identity-chain` (PR #25) against the outbox
